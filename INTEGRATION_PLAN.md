...
2. 付款方式选择 SimplePay 且没有现成参考号时，POS 建立待付款订单并显示二维码。
3. 顾客登录 SimplePay 后扫码，核对固定金额并确认付款。
4. SimplePay 完成扣款后自动回写 POS；关联的 RM180 联盟配套随后自动派发。
5. 到“设置 > 三系统融合准备”查看待确认、待关联和异常数量。
6. 出现 `needs-attention` 时由管理员核对，不会自动重复扣款或重复发奖励。
